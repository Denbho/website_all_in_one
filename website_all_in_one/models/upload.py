...
class sale_order(models.Model):
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        if set_qty:
            ...
            stock_type = self.env["ir.config_parameter"].sudo().get_param('website_sale_product_stock.stock_type')
            prod = self.env['product.product'].with_context(product_context).browse(int(product_id))
            if prod.type == 'product':
                if stock_type == 'available':
                    if prod.qty_available >= order_line.product_uom_qty + (add_qty or 0):
                        quantity = order_line.product_uom_qty + (add_qty or 0)
                    else:
                        order_line.product_uom_qty = prod.qty_available
                        quantity = order_line.product_uom_qty
                elif stock_type == 'outgoing':
                    if prod.virtual_available >= order_line.product_uom_qty + (add_qty or 0):
                        quantity = order_line.product_uom_qty + (add_qty or 0)
                    else:
                        order_line.product_uom_qty = prod.virtual_available
                        quantity = order_line.product_uom_qty
            else:
                quantity = order_line.product_uom_qty + (add_qty or 0)
